...
// For example, Validate.installPkg('eslint', '*', {'dev': true});
exports.installPkg = function (name, version, options, root) {
    ...
    var packagePath = Path.join(exports.findProjectRoot(root), 'package.json');
    var package = JSON.parse(Fs.readFileSync(packagePath, { encoding: 'utf8' }));
    if (!package.hasOwnProperty(dep) ||
        !package[dep].hasOwnProperty(name) ||
        options.overwrite) {
